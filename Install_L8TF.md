...
* [General options](#user-content-general-options)
  + [`--(no-)verbose`(`=`_level_) (`-v`)](#user-content---no-verboselevel--v)
* [Informational modes](#user-content-informational-modes)
  + [`--help` (`-?`, `-h`)](#user-content---help----h)
  + [`--version` (`-V`)](#user-content---version--v)
  + [`--about`](#user-content---about)
  + [`--args`](#user-content---args)
  + [`--dry-run` (`-n`)](#user-content---dry-run--n)
* [Directory path options](#user-content-directory-path-options)
  + [`--subject=`_path_](#user-content---subjectpath)
  + [`--l8tf-root=`_path_](#user-content---l8tf-rootpath)
  + [`--alltests=`_path_](#user-content---alltestspath)
  + [`--distcopy`(`=`_path_) vs. `--no-distcopy`](#user-content---distcopypath-vs---no-distcopy)
  + [`--run=`_path_](#user-content---runpath)
* [Subject-specific issues](#user-content-subject-specific-issues)
  + [`--subject-test-ac=`_path_](#user-content---subject-test-acpath)
  + [`--subject-marker=`_filename_](#user-content---subject-markerfilename)
  + [`--specs=`_path_](#user-content---specspath)
  + [`--unit-sources=`_path_](#user-content---unit-sourcespath)
  + [`--unit-prefix=`_string_](#user-content---unit-prefixstring)
* [Final actions](#user-content-final-actions)
  + [`--autoconf` vs. `--no-autoconf`](#user-content---autoconf-vs---no-autoconf)
  + [`--configure-tests` vs. `--no-configure-tests`](#user-content---configure-tests-vs---no-configure-tests)
* [Update options](#user-content-update-options)
  + [`--clobber` vs. `--no-clobber`](#user-content---clobber-vs---no-clobber)
  + [`--diff`(`=`_flags_) vs. `--no-diff`](#user-content---diffflags-vs---no-diff)
  + [`--recreate` vs. `--no-recreate`](#user-content---recreate-vs---no-recreate)
* [Initialization files](#user-content-initialization-files)
  + [`--no-init`](#user-content---no-init)
  + [`--site-init` vs. `--no-site-init`](#user-content---site-init-vs---no-site-init)
  + [`--subject-init` vs. `--no-subject-init`](#user-content---subject-init-vs---no-subject-init)
  + [`--prev-init` vs. `--no-prev-init`](#user-content---prev-init-vs---no-prev-init)
  + [`--user-init`(`=`_path_) vs. `--no-user-init`](#user-content---user-initpath-vs---no-user-init)

## General options

Options that are nearly always applicable:

### `--(no-)verbose`(`=`_level_) (`-v`)
...
### `--distcopy`(`=`_path_) vs. `--no-distcopy`
...
### `--user-init`(`=`_path_) vs. `--no-user-init`
